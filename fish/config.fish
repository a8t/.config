abbr ll 'ls -alF'
abbr la 'ls -A'
abbr l 'ls -aCF'
abbr projects 'c /Users/andytran/Documents/projects'
function c
    cd $argv; and echo "Now in:" (prompt_pwd); and ls
end

abbr .. 'c ../'
abbr ... 'c ../../'
abbr .... 'c ../../../'
abbr back 'c -'

function mcd
	mkdir -p $argv; and c $argv
end

abbr andc 'and c $_'

abbr nvim. 'nvim'
abbr vim 'nvim'
abbr vim. 'nvim'
abbr a. 'code'
abbr a 'code'

abbr fishrc 'code ~/.config/fish/config.fish'
abbr refish 'source ~/.config/fish/config.fish'

abbr vimrc 'touch ~/.config/nvim/init.vim; and nvim ~/.config/nvim/init.vim'
abbr revim 'nvim +PluginInstall'
abbr tmuxconf 'nvim ~/.tmuxconf'
abbr retmux 'tmux source ~/.tmuxconf'

abbr gi 'git init'
abbr ga 'git add ./'
abbr gc 'git commit -m'
abbr gp 'git push'
abbr gpl 'git pull'
abbr gs 'git status'
abbr gl 'git log'
abbr gch 'git checkout'
abbr gcb 'git checkout -b'
abbr gb 'git branch'
abbr grh 'git reset --hard'
abbr gf 'git fetch -p'
abbr gcp 'git cherry-pick'
abbr gm 'git merge'
abbr gm 'git merge --no-ff'
abbr gcm 'git checkout master'
abbr grbo 'git rebase origin/master'
abbr gro 'git rebase origin/master'
abbr grom 'git rebase origin/master'
abbr gst 'git stash -k -u'
abbr gsta 'git stash apply'
abbr gstl 'git stash list'
abbr gstc 'git stash clear'
abbr grono 'git rebase --onto'


function gitignore
	touch .gitignore; and echo "node_modules/" >> .gitignore
end

# function nvm
#       bass source ~/.nvm/nvm.sh ';' nvm $argv
#   end
abbr finder 'open ./'

function getlocalip
  ifconfig | grep -Eo 'inet (addr:)?([0-9]*\.){3}[0-9]*' | grep -Eo '([0-9]*\.){3}[0-9]*' | grep -v '127.0.0.1'
end

abbr sethost 'set -x HOST (eval getlocalip)'

set -gx PATH $HOME/.cargo/bin $PATH
set -gx PATH $PATH /opt/homebrew/bin
set -gx ANDROID_HOME $HOME/Library/Android/sdk
set -gx PATH $PATH $ANDROID_HOME/emulator
set -gx PATH $PATH $ANDROID_HOME/emulator/emulator
set -gx PATH $PATH $ANDROID_HOME/tools
set -gx PATH $PATH $ANDROID_HOME/tools/bin
set -gx PATH $PATH $ANDROID_HOME/platform-tools
set -gx PATH $PATH $HOME/dev/ddcctl
set -gx PATH $PATH $HOME/bin

bind \e\[1\;5C forward-word
bind \e\[1\;5D backward-word



source /opt/homebrew/Cellar/asdf/0.9.0/asdf.fish
set -q GHCUP_INSTALL_BASE_PREFIX[1]; or set GHCUP_INSTALL_BASE_PREFIX $HOME ; set -gx PATH $HOME/.cabal/bin /Users/andy/.ghcup/bin $PATH # ghcup-env

function naboo_logs
 if test (count $argv) -lt 1
     kubectl logs --tail 300 -n naboo -f (kubectl get pod -l "app=naboo-staging" -o name -n naboo | sed "s/pod\///" | head -n 1) -c naboo
 else
     kubectl logs --tail 300 -n naboo -f (kubectl get pod -l "app=naboo-$argv[1]" -o name -n naboo | sed "s/pod\///" | head -n 1) -c naboo
 end
end

function naboo_console
 if test (count $argv) -lt 1
     kubectl exec -n naboo -it (kubectl get pod -l "app=naboo-staging" -o name -n naboo | sed "s/pod\///" | head -n 1) -c naboo -- /opt/app/bin/naboo remote 
 else
     kubectl exec -n naboo -it (kubectl get pod -l "app=naboo-$argv[1]" -o name -n naboo | sed "s/pod\///" | head -n 1) -c naboo -- /opt/app/bin/naboo remote 
 end
end